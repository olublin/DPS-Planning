...
sys.stdout = open("gravity_model_dsa_half.log", "w")
...
dps_base_hs.loc[0,'geometry'] = hs_full_geo.loc[583,'geometry'].centroid
...
capacities = [500,1340,1335,1400,1240,1510,1550]
...
def score_candidate(candidate,sgr,lower_bound,upper_bound):
    ...
    local_dps_base_hs.loc[6,'geometry']=candidate_geom.centroid
...
dps_pu['score'] = dps_pu.apply(lambda row:score_candidate(row,15,80,100),axis=1)

dps_pu.to_file('gravity_dsa_half.geojson', driver='GeoJSON')